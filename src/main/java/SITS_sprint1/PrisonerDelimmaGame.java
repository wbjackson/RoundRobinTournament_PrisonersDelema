...
public class PrisonerDelimmaGame extends AGame
{
    ...
    public Robot playGame(Robot p1, Robot p2)
    {
        ...
        {
            if (p1.isHumanControlled())
            {
                notifyMoveObserver("Waiting for " + p1.getName() + " to make a move...");
            }

            String move1 = p1.makeMove();

            if (p2.isHumanControlled())
            {
                notifyMoveObserver("Waiting for " + p2.getName() + " to make a move...");
            }
